    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild className="text-sm md:text-base">
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="justify-between w-[140px] md:w-[180px] text-black"
        >
          {value ? datas.find((data) => data.name === value)?.name : title}
          <ChevronsUpDown className="opacity-50 ml-2 w-4 h-4 shrink-0" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="p-0 w-[140px] md:w-[180px] max-h-60 text-black text-sm md:text-base overflow-y-auto">
        <Command>
